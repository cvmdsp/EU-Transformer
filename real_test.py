...
with torch.no_grad():
    ...
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        ...
        rgb_noisy1 = rgb_noisy
        _, _, h, w = rgb_noisy.shape
        filenames = data_test[1]
        rgb_noisy, mask = expand2square(rgb_noisy, factor=128)
        rgb_restored = model_restoration(rgb_noisy)
        rgb_restored = torch.masked_select(rgb_restored,mask.bool()).reshape(1,1,h,w)
        # rgb_restored = torch.clamp(rgb_restored,0,1).cpu().numpy().squeeze().transpose((1,2,0))
        rgb_restored = torch.clamp(rgb_restored, 0, 1).cpu().numpy().transpose((1,2,3,0))
        rgb_restored =rgb_restored.squeeze()
        ...
        # rgb_noisy = rgb_noisy1.squeeze().cpu()
        # denoised_img = rgb_restored
        #
        # plt.subplot(), plt.imshow(rgb_noisy / denoised_img, cmap='gray', vmin=0, vmax=4), plt.title(filenames[0])
        # plt.savefig('/media/dell/2.0/wsy/Uformer-main/val/real/ratio image/filenames[0].png')
